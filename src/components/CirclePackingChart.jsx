...
import data from "./workforce_hierarchy.json";
...
export default function CirclePackingChart() {
  ...
  return (
    <>
      {/* 🌟 تعديل الـ CSS للتحكم في مكان المصدر والاسم حسب نوع الشاشة */}
      <style>{`
        .app-container {
          display: flex;
          width: 100%;
        ...
        /* يظهر في الديسكتوب ويختفي في الجوال */
        .source-credits-desktop {
          display: flex;
          flex-direction: column;
          gap: 6px;
          margin-top: auto;
        ...
        /* يختفي في الديسكتوب ويظهر في الجوال */
        .source-credits-mobile {
          display: none;
        }

        /* 📱 إعدادات الاستجابة للآيفون والشاشات الصغيرة (أصغر من 992px) */
        @media (max-width: 992px) {
          ...
          /* إخفاء المصدر من القسم العلوي في الجوال */
          .source-credits-desktop {
            display: none;
          }
          
          /* إظهار المصدر في نهاية الصفحة تماماً (أسفل الدائرة) مع التنسيق */
          .source-credits-mobile {
            display: flex;
            flex-direction: column;
            gap: 6px;
            width: 100%;
            text-align: center; /* توسيط النص ليتناسق مع الدائرة في الجوال */
      ...
      <div className="app-container" onClick={() => setIsDropdownOpen(false)}>
        {/* القسم الأيمن (النصوص والبحث) */}
        <div className="right-section" onClick={(e) => e.stopPropagation()}>
          ...
          >
            ...
            يمكنك النقر على الدوائر للتعمق في التفاصيل (Zoom-in)، أو استخدام
            محرك البحث السريع للوصول المباشر لنشاط محدد.
          </p>
          ...
          {/* 💻 المصدر المخصص للشاشات الكبيرة (يختفي تلقائياً على الجوال) */}
          <div className="source-credits-desktop">
            {/* <div>
              تطوير وإعداد:{" "}
        ...
        >
                ...
                d.descendants().forEach((node) => {
                  if (!node.children && node.data.stats) {
                    node.data.stats.forEach((s) => {
                      if (
                        s.label.includes("سعودي") &&
                        !s.label.includes("غير")
                      ) {
                        saudiTotal += s.value;
          ...
          {/* 📱 المصدر المخصص للجوال (يظهر أسفل الدائرة تماماً في الآيفون ويختفي في الكمبيوتر) */}
          <div className="source-credits-mobile">
            {/* <div>
              تطوير وإعداد:{" "}
              <strong style={{ color: "#555" }}>د. أنس ال حرشان</strong>
            </div> */}
